...
class _SessionDetailViewState extends State<SessionDetailView> with SingleTickerProviderStateMixin {
  final SessionDetailController controller = Get.find<SessionDetailController>();
  late TabController _tabController;

  @override
  void initState() {
    super.initState();
    _tabController = TabController(length: 2, vsync: this, initialIndex: controller.selectedTabIndex.value);
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Transform.translate(
                    offset: const Offset(0.0, 0.0), // Adjust this value to move the icon up/down
                    child: IconButton(
                      icon: Image.asset(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    if (index < controller.participants.length) {
                      ...
                      Color borderColor;
                      if (user.name == 'Mark') {
                        borderColor = const Color(0xFF2F80ED); // Specific blue for Mark
                      } else if (user.name == 'John') {
                        borderColor = const Color(0xFFEB5757); // Specific red for John
                      } else if (user.name == 'Steve') {
                        borderColor = const Color(0xFF27AE60); // Specific green for Steve
                      } else {
                        borderColor = Colors.transparent; // Default or no border for others
                      }
                      return Padding(
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    const Spacer(),
                                    const Icon(Icons.swap_vert, color: Color(0xFF222222), size: 20),
                                  ],
                                ),
                                              ...
                                              Expanded(
                                                child: Column(
                                                  ...
                                                  children: [
                                                    Row(
                                                      children: [
                                                        if (recording.status == 'New Recording')
                                                          Padding(
                                              ...
                                              Column(
                                                ...
                                                children: [
                                                  if (recording.user != null)
                                                    CircleAvatar(
                                                      radius: 16,
                                                      backgroundImage: NetworkImage(recording.user!.avatarUrl),
                                                      backgroundColor: Colors.white,
                                                      child: Container(
                                                        decoration: BoxDecoration(
                                                          shape: BoxShape.circle,
                                                          border: Border.all(color: (recording.user!.name == 'Mark') ? const Color(0xFF2F80ED) : (recording.user!.name == 'John') ? const Color(0xFFEB5757) : (recording.user!.name == 'Steve') ? const Color(0xFF27AE60) : Colors.transparent, width: 1.5),
                                                        ),
                                                      ),
                                                    ),
                                                  const SizedBox(height: 4),
                                                    ...
                                                    children: [
                                                      if (recording.status == 'Recording...')
                                                        Image.asset(
                                                          ...
                                                          width: 14,
                                                          height: 14,
                                                          color: const Color(0xFFEB5757), // Apply red color to the image
                                                        ),
                                                      if (recording.status == 'Recording...')
                                                      ...
                                                      Image.asset(
                                                        ...
                                                        width: 14,
                                                        height: 14,
                                                        color: const Color(0xFFBDBDBD), // Apply red color to the image
                                                      ),
                                                      if (recording.status == 'completed')
                                                        const SizedBox(width: 4),
                                                      Text(
                                                        recording.status == 'Recording...' ? 'Recording...' : recording.duration!,
                                                        style: TextStyle(
                                                          color: recording.status == 'Recording...' ? const Color(0xFFEB5757) : const Color(0xFFBDBDBD),
                                                          fontSize: 12,
